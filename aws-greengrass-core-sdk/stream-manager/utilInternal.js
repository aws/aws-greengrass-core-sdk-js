...
const intToBuffer = (i, length = 4) => {
    ...
    if (length === 4) {
        ...
        throw new Error('Illegal value specified for intToBuffer length. Must be 1, 2 or 4.');
    }
    return buf;
};
...
const removeEmptyArray = (nam, val) => {
    if (val && val.length === 0) {
...
const serializeToJsonWithEmptyArrayAsNull = (o) => Buffer.from(JSON.stringify(o, removeEmptyArray));
...
const throwOnErrorResponse = (response) => {
    ...
    } else if (response.status === smData.ResponseStatusCode.UpdateFailed) {
        throw new exceptions.UpdateFailedException(response.errorMessage, response.status, response.requestId);
    } else if (response.status === smData.ResponseStatusCode.UpdateNotAllowed) {
        throw new exceptions.UpdateNotAllowedException(response.errorMessage, response.status, response.requestId);
    } else if (response.status === smData.ResponseStatusCode.UnknownOperation) {
        throw new exceptions.UnknownOperationException(response.errorMessage, response.status, response.requestId);
    } else {
